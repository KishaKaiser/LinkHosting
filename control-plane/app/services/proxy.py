...
_PORT_RE = re.compile(r"^[1-9]\d{0,4}$")
...
{client_max_body_size_line}
    ...
    location ^~ /api/proxy/ {{
        ...
        proxy_set_header X-Forwarded-For $proxy_add_x_forwarded_for;
        proxy_set_header X-Forwarded-Proto $scheme;
        # AI report/interpretation generation can run well past nginx's 60s
        # default before the api container responds — match the generous
        # timeout already used for non-PL_CMS sites.
        proxy_read_timeout 300s;
        proxy_connect_timeout 300s;
        proxy_send_timeout 300s;
    }}
...
def _container_port(site: Site) -> int:
    ...
    if site.site_type == SiteType.node:
        env = _site_env_vars(site)
        for key in (_NODE_UPSTREAM_PORT_ENV_KEY, "PORT"):
            value = str(env.get(key, "")).strip()
            if _PORT_RE.match(value):
                port = int(value)
                if 1 <= port <= 65535:
                    return port
    ports = {
...
def _upstream_name(site: Site) -> str:
    ...
    if site.site_type == SiteType.pl_cms:
        from app.services.pl_cms import get_pl_cms_container_name
        return get_pl_cms_container_name(site.name, "web")
    return f"site-{site.name}"


def _client_max_body_size_line(site: Site) -> str:
    env_vars = _site_env_vars(site)
...
def _site_env_vars(site: Site) -> dict[str, str]:
...
def write_vhost(site: Site, tls: bool = False) -> Path:
    ...
    if site.site_type == SiteType.pl_cms:
        ...
        template = VHOST_TEMPLATE_PL_CMS_HTTPS if tls else VHOST_TEMPLATE_PL_CMS_HTTP
        content = template.format(
            site_name=site.name,
            domain=site.domain,
            web_upstream=get_pl_cms_container_name(site.name, "web"),
            api_upstream=get_pl_cms_container_name(site.name, "api"),
