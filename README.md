...
4. The worker:
   ...
   - For one-click/template deploys, auto-imports `KishaKaiser/PL_CMS` into the site directory when the PL_CMS monorepo is missing
   - Validates the PL_CMS pnpm monorepo build context after source import and before invoking `docker compose up`
